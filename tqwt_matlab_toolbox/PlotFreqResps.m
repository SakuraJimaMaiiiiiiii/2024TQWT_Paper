...
txt1 = sprintf('参数信息: Q = %1.2f, R = %3.2f', Q,r);%txt1 = sprintf('FREQUENCY RESPONSES: Q = %1.2f, R = %3.2f', Q,r);
title(txt1)
if nargin < 4
    xlabel('归一化频率 (Hz)')% xlabel('NORMALIZED FREQUENCY (HERZ)')
else
    xlabel('频率 (Hz)')%xlabel('FREQUENCY (HERZ)')
end
